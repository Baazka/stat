...
function Mayagt_files(props) {
  ...
  async function fetchData() {
    DataRequest({
      url: statUrl + "getFile?StatID=" + mayagtData.ID,
      method: "GET",
      data: {},
    })
    ...
    if (file.target.files[0].size > 100000000) {
      ...
      axios
        ...
        .then(async function (resultFile) {
          if (
            (resultFile.data.status === 413 &&
              resultFile.data.message === "100MB-с их хэмжээтэй байна!") ||
            resultFile.status === 413
          ) {
            alert("файл 100MB-с их хэмжээтэй байна");
            setloaderSpinner(0);
          } else if (resultFile.data.filePath !== undefined) {
            let resultUplaod = await axios.post(statUrl + "postFile/", {
              file: {
                ID: null,
  ...
  async function deleteComment(value) {
    if (fileAdd.FILE_PATH !== undefined) {
      let resultFile = await axios.delete(
        statUrl + "removeFile/" + mayagtData.ID + "/" + fileAdd.FILE_NAME,
        {}
      );
      ...
      if (resultFile.data.message === "Файлыг устгалаа.") {
        ...
        if (resultUplaod.data !== undefined) fetchData();
      }
    } else {
      fetchData();
    }
    ...
    <div>
      {loaderSpinner === 1 || loaderSpinner === undefined ? (
        ...
        <div className="flex flex-row">
          <div className=" uppercase  mr-1">Хавсралт оруулах:</div>
          ...
          {fileAdd?.FILE_PATH !== undefined && fileAdd?.FILE_PATH !== "" ? (
            <a
              href={
                statUrl + "uploads" + fileAdd?.FILE_PATH.replace("uploads", "")
              }
