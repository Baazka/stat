...
function CM_9D() {
  ...
  const columns = React.useMemo<ColumnDef<Stat_CM9D, any>[]>(
    () => [
      {
  ...
  let Stat_CM9D = [{}];
  const [data, setData] = React.useState<Stat_CM9D[]>(Stat_CM9D);
    ...
    <>
      ...
      >
        <div className="justify-start flex mb-2 mt-2">
          <Title
            title={
              "ТӨРИЙН АУДИТЫН БАЙГУУЛЛАГЫН СУРГАЛТ, ХӨГЖЛИЙН ҮЙЛ АЖИЛГААНЫ ХЭРЭГЖИЛТ 3-ТАБСМ-9Г"
            }
            widthS={"49rem"}
            widthL={"14rem"}
          />
          <div className="mt-1 ml-1.5">
            <Subtitle mayagtName={"З-ТАБСМ-9Г"} />
          </div>
        </div>
