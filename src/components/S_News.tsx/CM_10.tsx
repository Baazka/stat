...
function CM_10() {
  ...
  const columns = React.useMemo<ColumnDef<Stat_CM10, any>[]>(
    () => [
      {
      ...
      },
      {
        accessorKey: "AUDIT_KOD",
        cell: (info) => info.getValue(),
        header: "Аудитын хуулийн этгээдийн тоо",
        footer: (props) => props.column.id,
      },
      {
        accessorKey: "AUDIT_KOD",
        cell: (info) => info.getValue(),
        header: "Аудитын үйлчилгээний төлбөр /сая төгрөг/",
        footer: (props) => props.column.id,
      },
      {
        accessorKey: "AUDIT_KOD",
  ...
  let Stat_CM10 = [{}];
  const [data, setData] = React.useState<Stat_CM10[]>(Stat_CM10);
    ...
    <>
      ...
      >
        <div className="justify-start flex mb-2 mt-2">
          <Title
            title={
              "ТАЙЛАНТ ОНД АУДИТАД ГЭРЭЭНИЙ ҮНДСЭН ДЭЭР ОРОЛЦСОН ШИНЖЭЭЧ 3-ТАБСМ-10"
            }
            widthS={"43rem"}
            widthL={"11rem"}
          />
          <div className="mt-1 ml-1.5">
            <Subtitle mayagtName={"З-ТАБСМ-10"} />
          </div>
        </div>
