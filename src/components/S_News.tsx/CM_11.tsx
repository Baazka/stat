...
function CM_11() {
  ...
  const columns = React.useMemo<ColumnDef<Stat_CM11, any>[]>(
    () => [
      {
      ...
      },
      {
        accessorKey: "AUDIT_KOD",
        cell: (info) => info.getValue(),
        header: "Өмнөх оны зөвлөмжийн биелэлт",
        footer: (props) => props.column.id,
      },
      {
        accessorKey: "AUDIT_KOD",
      ...
      },
      {
        accessorKey: "AUDIT_KOD",
        cell: (info) => info.getValue(),
        header: "Шийдвэрлэсэн өргөдөл, гомдлын тоо",
        footer: (props) => props.column.id,
      },
      {
        accessorKey: "AUDIT_KOD",
  ...
  let Stat_CM11 = [{}];
  const [data, setData] = React.useState<Stat_CM11[]>(Stat_CM11);
    ...
    <>
      ...
      >
        <div className="justify-start flex mb-2 mt-2">
          <Title
            title={"ДОТООД АУДИТ 3-ТАБСМ-11"}
            widthS={"12rem"}
            widthL={"6rem"}
          />
          <div className="mt-1 ml-1.5">
            <Subtitle mayagtName={"З-ТАБСМ-11"} />
          </div>
        </div>
