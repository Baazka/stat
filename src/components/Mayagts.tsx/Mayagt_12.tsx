...
function Mayagt_9A(props: any) {
  ...
  function CustomCell ({ getValue, row, column: { id }, table,drop:{drop1:[],drop2:[]}}){
    const initialValue = getValue()
    // We need to keep and update the state of the cell normally
    const [value, setValue] = React.useState(initialValue)
  ...
  async function fetchData() {
   DataRequest({
      ...
      DataRequest({
        ...
        if(refMovementType.data !== undefined && refMovementType?.data.length>0){
          let temp = drop
          ...
          DataRequest({
            ...
            .then(function (refMovementSubType) {
              if(refMovementSubType.data !== undefined && refMovementSubType?.data.length>0){
