...
function Mayagt_5(props: any) {
  ...
  const columns = React.useMemo<ColumnDef<Stat_m5, any>[]>(
    () => [
      ...
      {
        accessorKey: "AUDITR_KOD",
        header: "Аудиторын код",
        cell: (info) => info.getValue(),
      },
      {
        accessorKey: "ZORCHIL_ARILGH_BARIMT_OGNOO",
      ...
      {
        accessorKey: "BURTGELEES_HASAGDSN_DUGAAR",
      ...
      {
        accessorKey: "TOLOWLOSON_SANHUU_UR_OGOOJ",
        header: "Төлөвлөсөн санхүүгийн үр өгөөжийн дүн (төгрөг)",
        cell: (info) => info.getValue(),
      },
      {
        accessorKey: "TODORHOILSON_SANHUUGIIN_UR_OGOOJ_DUN",
  ...
  let Stat_m5 = [{}];
  const [data, setData] = React.useState<Stat_m5[]>(Stat_m5);
  ...
  return (
    <>
      ...
      >
        <Title
          title={"АЛБАН ШААРДЛАГЫН БҮРТГЭЛ З-ТАББМ-5"}
          widthS={"18rem"}
          widthL={"9rem"}
        />
