...
function Excel() {
  ...
  const schema = [
    ...
    {
      column: "Аудитын код dasd sa asd as dasd asd ad as asd as d",
      wrap: true,
      width: 30,
      type: String,
      value: (student: any) => student.Name,
    ...
    // },
    // {
    //   column: "Алдаа, зөрчлийн дэд анги",
    //   wrap: true,
    //   sticky: "left",
    //   width: 30,
    //   type: String,
    ...
    // },
    // {
    //   column: "Үр өгөөжөөр тооцох эсэх",
    //   wrap: true,
    //   sticky: "left",
    //   width: 30,
    //   type: String,
...
type Stat_m3 = {
  ...
  AUDIT_KOD: number;
  AUDIT_HH_HELBER: string;
  BAIGUULLGA_NER: string;
  REGISTER: number;
  TOSOW_ZAHIRGCH_ANGILL: string;
  TUHAIN_UR_DUNG_AZTE: string;
  ALDAAG_ZALRUULSAN_ESEH: string;
  ALDAANII_TOVCH_UTGA: string;
  ALDAANII_DUN_T: string;
  ZALRUULSAN_ALDAANII_ANGILL: string;
  UR_OGOOJ_TOOTSOH_ESEH: string;
  UR_OGOOJIIN_TOROL: string;
};
...
function Mayagt_3() {
  ...
  const columns = React.useMemo<ColumnDef<Stat_m3, any>[]>(
    () => [
      {
      ...
      {
        accessorKey: "ALDAAG_ZALRUULSAN_ESEH",
        header: "Алдааг залруулсан эсэх",
        cell: (info) => info.getValue(),
      },
      {
        accessorKey: "TOVCH_UTGA",
        header: "Tовч утга",
        cell: (info) => info.getValue(),
      },
      {
      ...
      {
        accessorKey: "ALDAA_ZORCHILIIN_DED_ANGI",
  ...
  let Stat_m3 = [{}];
  const [data, setData] = React.useState<Stat_m3[]>(Stat_m3);
