...
function Mayagt_15_Create(props: any) {
  ...
  const [data, loadData] = useState({
    ...
    SHORT_DESC:"",
    INFO_TYPE_ID:999,
    IS_FULFILLMENT:999, //1-БИЕЛСЭН 2-БИЕЛЭЭГҮЙ
    RECOMMENDATION_TYPE_ID:999,
    DECIDED_COUNT:0,
  ...
  async function fetchData() {
    ...
    let refInfoType = await axios(Stat_Url + "refInfoType");
    if (refInfoType.data !== undefined && refInfoType.data.length > 0) {
      let temp = drop;
      temp.drop3 = refInfoType.data;
      setDrop({ ...temp });
    }
    let refRecommendationType = await axios(Stat_Url + "refRecommendationType");
    if (refRecommendationType.data !== undefined && refRecommendationType.data.length > 0) {
      let temp = drop;
      temp.drop4 = refRecommendationType.data;
      setDrop({ ...temp });
    }
  ...
  function requiredField(){
   if(data.DEPARTMENT_ID === 999){
    ...
    return false;
   }else if(data.RECOMMENDATION_YEAR === 0){
    alert('Зөвлөмж өгсөн хугацаа оруулна уу')
    return false;
   }else if(data.INFO_TYPE_ID === 999){
    alert('Зөвлөмжийн мэдээллийн төрөл сонгоно уу')
    return false;
   }else if(data.IS_FULFILLMENT === 999){
    alert('Зөвлөмж биелсэн эсэх сонгоно уу')
    return false;
   }else if(data.RECOMMENDATION_TYPE_ID === 999){
    alert('Зөвлөмжийн хугацаа (Зөвхөн зөвлөмж биелсэн эсэх дээр үгүй гэж сонгосон тохиолдолд сонгосон.) сонгоно уу')
    return false;
   }
   else if(data.EVENT_NAME === '' || data.EVENT_NAME === null ){
    alert('ХШҮ хийсэн арга хэмжээний нэр оруулна уу')
    return false;
   }
   else if(data.EVENT_NAME === '' || data.EVENT_NAME === null ){
    alert('ХШҮ хийсэн арга хэмжээний нэр оруулна уу')
    ...
    alert('Зөвлөмжийн утга оруулна уу')
    return false;
   }
   else if(data.UNIMPLEMENTED_REASON === '' || data.UNIMPLEMENTED_REASON === null ){
    alert('Зөвлөмжийн хэрэгжээгүй шалтгаан тайлбар оруулна уу')
    return false;
   }
   else {
    return true
  ...
  return (
    ...
    >
      ...
      >
        <Title title={"З-ТАББМ-11"} widthS={"5rem"} widthL={"5rem"} />
      </div>
      <div className="ml-20 bg-blue-500 w-48 h-10 rounded-lg mt-6">
        <div className="space-y-4">
          <p className="text-white text-center  pt-2">З-ТАББМ-11</p>
        </div>
      </div>
          ...
          <div className="flex  md:justify-center sm:justify-end">
              <div className="grid grid-rows-4  lg:grid-flow-col  sm:grid-flow-row ">
                ...
                <div className="flex space-x-4 space-x-reverse h-10 my-1">
                  <div className="w-6/12">
                    <label className="block md:text-right md:mb-10  pr-6">
                      <span className="text-md">Зөвлөмж өгсөн хугацаа /оноор/:</span>
                    </label>
                  </div>
                  <div className="w-6/12 ">
                  <input
                      ...
                      value={data.RECOMMENDATION_YEAR}
                      onChange={(e) => {
                        let temp = data;
                        temp.RECOMMENDATION_YEAR = e.target.value;
                        if(temp.RECOMMENDATION_YEAR.length <= 4){
                            loadData({
                                ...temp,
                              });
                        }
                ...
                <div className="flex space-x-4 space-x-reverse h-10 my-1">
                  <div className="w-6/12">
                    <label className="block md:text-right md:mb-10  pr-6">
                      <span className="text-md">ХШҮ хийсэн арга хэмжээний нэр:</span>
                    </label>
                  </div>
                  <div className="w-6/12 ">
                  <input
                      ...
                      onChange={(e) => {
                        let temp = data;
                        temp.EVENT_NAME = e.target.value;
                        loadData({
                          ...temp,
                        });
                      }}
                ...
                <div className="flex space-x-4 space-x-reverse h-10 my-1">
                  <div className="w-6/12">
                    <label className="block md:text-right md:mb-10  pr-6">
                      <span className="text-md">Зөвлөмжийн утга:</span>
                    </label>
                  </div>
                  <div className="w-6/12 ">
                  <textarea
                ...
                <div className="flex space-x-4 space-x-reverse h-10 my-1">
                  <div className="w-6/12">
                    <label className="block md:text-right md:mb-10  pr-6">
                      <span className="text-md">Зөвлөмжийн мэдээллийн төрөл:</span>
                    </label>
                  </div>
                  <div className="w-6/12 ">
                    <select
                      ...
                      onChange={(value) => {
                        let temp = data;
                        temp.INFO_TYPE_ID = value.target.value;
                        loadData({ ...temp });
                      }}
                    >
                      <option value={999}>Бүгд</option>
                      {drop.drop3.map((nation, index) => (
                        <option
                          className="font-semibold"
                          key={nation.INFO_TYPE_NAME}
                          value={nation.INFO_TYPE_ID}
                        >
                          {nation.INFO_TYPE_NAME}
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                <div className="flex space-x-4 space-x-reverse h-10 my-1">
                  <div className="w-6/12">
                    <label className="block md:text-right md:mb-10  pr-6">
                      <span className="text-md">Зөвлөмж биелсэн эсэх:</span>
                    </label>
                  </div>
                  <div className="w-6/12 ">
                  <select
                      ...
                      value={data.IS_FULFILLMENT}
                      onChange={(value) => {
                        let temp = data;
                        temp.IS_FULFILLMENT = value.target.value;
                        loadData({ ...temp });
                      }}
                    >
                      <option value={999}>Бүгд</option>
                ...
                <div className="flex space-x-4 space-x-reverse h-10 my-1">
                  <div className="w-6/12">
                    <label className="block md:text-right md:mb-10  pr-6">
                      <span className="text-md">Зөвлөмжийн хугацаа (Зөвхөн зөвлөмж биелсэн эсэх дээр үгүй гэж сонгосон тохиолдолд сонгосон.):</span>
                    </label>
                  </div>
                  <div className="w-6/12 ">
                    <select
                      ...
                      value={data.RECOMMENDATION_TYPE_ID}
                      onChange={(value) => {
                        let temp = data;
                        temp.RECOMMENDATION_TYPE_ID = value.target.value;
                        loadData({ ...temp });
                      }}
                    >
                      <option value={999}>Бүгд</option>
                      {drop.drop4.map((nation, index) => (
                        <option
                          className="font-semibold"
                          key={nation.RECOMMENDATION_TYPE_NAME}
                          value={nation.RECOMMENDATION_TYPE_ID}
                        >
                          {nation.RECOMMENDATION_TYPE_NAME}
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                <div className="flex space-x-4 space-x-reverse h-10 my-1">
                  <div className="w-6/12">
                    <label className="block md:text-right md:mb-10  pr-6">
                      <span className="text-md">Шийдвэрлэгдсэн  өргөдөл гомдлын  тоо:</span>
                    </label>
                  </div>
                  <div className="w-6/12 ">
                  <input
                      ...
                      value={data.DECIDED_COUNT}
                      onChange={(e) => {
                        let temp = data;
                        temp.DECIDED_COUNT = e.target.value;
                        loadData({
                          ...temp,
                        });
                      }}
                ...
                <div className="flex space-x-4 space-x-reverse h-10 my-1">
                  <div className="w-6/12">
                    <label className="block md:text-right md:mb-10  pr-6">
                      <span className="text-md">Зөвлөмжийн хэрэгжээгүй шалтгаан тайлбар:</span>
                    </label>
                  </div>
                  <div className="w-6/12 ">
                  <textarea
                      ...
                      value={data.UNIMPLEMENTED_REASON}
                     
                      onChange={(e) => {
                        let temp = data;
                        temp.UNIMPLEMENTED_REASON = e.target.value;
                        loadData({
                          ...temp,
                        });
                      }}
...
export default Mayagt_15_Create;
