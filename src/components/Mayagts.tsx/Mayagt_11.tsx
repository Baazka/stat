...
function Mayagt_11(props: any) {
  ...
  const columns = React.useMemo(
    () => [
      ...
      {
        accessorKey: "BUDGET_LEVEL_NAME",
        header: "Төсвийн төрөл",
        cell: (info) => info.getValue(),
      },

      {
        accessorKey: "CONCLUSION_FORM_NAME",
        header: "Дүгнэлтийн хэлбэр",
        cell: (info) => info.getValue(),
      },
      {
        accessorKey: "IS_REFLECT",
        header: "Тусгасан эсэх",
        cell: (info) => info.getValue(),
        accessorFn: (row, index) => (row.IS_REFLECT === 1 ? "Тийм" : "Үгүй"),
      },
      {
        accessorKey: "CONCLUSION_TYPE_NAME",
        header: "Дүгнэлтийн төрөл",
        cell: (info) => info.getValue(),
      },
      {
        accessorKey: "BENEFIT_TYPE_NAME",
        header: "Үр өгөөжийн төрөл",
        cell: (info) => info.getValue(),
      },
      {
        accessorKey: "BENEFIT_FIN_AMOUNT",
        header: "Санхүүгийн үр өгөөжийн дүн(төгрөг)",
        cell: (info) => info.getValue(),
        accessorFn: (row, index) => (
          <div>
            <CurrencyInput
              id="input-example"
              defaultValue={row.BENEFIT_FIN_AMOUNT}
              decimalsLimit={2}
              decimalScale={2}
              disabled
      ...
      {
        accessorKey: "BENEFIT_NONFIN_AMOUNT",
        header: "Санхүүгийн бус үр өгөөжийн дүн(төгрөг)",
        cell: (info) => info.getValue(),
        accessorFn: (row, index) => (
          <div>
            <CurrencyInput
              id="input-example"
              defaultValue={row.BENEFIT_NONFIN_AMOUNT}
              decimalsLimit={2}
              decimalScale={2}
              disabled
              style={{ textAlign: "center", backgroundColor: "transparent" }}
              ...
              <>
                ...
                >
                  <img
                    src={editPencil}
                    onClick={() => {
                      localStorage.removeItem("stat_bm8a");
                      Navigate("/web/Home/bm8a/create");
                      localStorage.setItem("stat_bm8a", JSON.stringify(row));
                    }}
                    width="14px"
                    height="14px"
                    alt=""
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="text-white p-2 rounded-md justify-end ml-4"
              onClick={() => deletePlan()}
            >
              Устгах
            </button>
